...
__global__ void v0_gemm_playground(const half* A, const half* B, half* C,
                               int M, int N, int K, float alpha, float beta) {
    using namespace v0_dims;   

    __shared__ half tile_a[2][M_SMEM_ROWS][WMMA_M];
    __shared__ half tile_b[2][WMMA_N][N_SMEM_COLS];
    int g_tile_id = 0;
    ...
    for (int k = 0; k < K; k += 16) {
        ...
        for (int i = 0; i < M_TILES; ++i) wmma::load_matrix_sync(a_frag[i], &tile_a[g_tile_id][tile_warp_row + i * WARP_DIM_Y * WMMA_M][0], WMMA_M);
        for (int j = 0; j < N_TILES; ++j) wmma::load_matrix_sync(b_frag[j], &tile_b[g_tile_id][0][tile_warp_col + j * WARP_DIM_X * WMMA_N], N_SMEM_COLS);
