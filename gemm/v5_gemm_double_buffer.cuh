...
__global__ void v5_gemm_double_buffer(const float* __restrict__ A, const float* __restrict__ B, float *C, int M, int K, int N, float alpha, float beta) {
    ...
    for (int k = Bk; k < K + Bk; k += Bk) {
        ...
        for (int p = 0; p < Bk; ++p) {
            ...
            for (int i = 0; i < Tm / 4; ++i) {
                ...
                FLOAT4(Areg[i * 4]) = FLOAT4(tile_a[tile_id][p][col ^ ((p >> 2) * (32 / (Bk / 4)))]);
            }
