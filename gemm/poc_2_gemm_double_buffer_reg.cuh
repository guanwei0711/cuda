...
__global__ void poc_2_gemm_double_buffer_reg(const float* __restrict__ A, const float* __restrict__ B, float *C, int M, int K, int N, float alpha, float beta) {
    ...
    for (int k = Bk; k < Bk + K; k += Bk) {
        ...
        for (int i = 0; i < Tm / 4; ++i) {
            ...
            FLOAT4(Areg[0][i * 4]) = FLOAT4(tile_a[tile_id][0][col]);
        }
