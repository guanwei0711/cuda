...
int main(int argc, char** argv) {
...

    {
        ...
        dim3 blocks((N + Bn - 1) / Bn, (M + Bm - 1) / Bm);
        cudaMemcpy(dC, hC.data(), sizeof(float) * sizeC, cudaMemcpyHostToDevice);
        v6_gemm_double_buffer<Bm, Bn, Bk, Tm, Tn, THREADS, 2><<<blocks, threads>>>(dA, dB, dC, M, K, N, alpha, beta);
        cudaDeviceSynchronize();
        if (check_correctness) {
            cudaMemcpy(hC_kernel.data(), dC, sizeof(float) * sizeC, cudaMemcpyDeviceToHost);
            float err = max_abs_error(hC_cpu, hC_kernel);
...

    {
        ...
        dim3 blocks((N + Bn - 1) / Bn, (M + Bm - 1) / Bm);
        cudaMemcpy(dC, hC.data(), sizeof(float) * sizeC, cudaMemcpyHostToDevice);
        v6_gemm_double_buffer<Bm, Bn, Bk, Tm, Tn, THREADS, 3><<<blocks, threads>>>(dA, dB, dC, M, K, N, alpha, beta);
        cudaDeviceSynchronize();
        if (check_correctness) {
            cudaMemcpy(hC_kernel.data(), dC, sizeof(float) * sizeC, cudaMemcpyDeviceToHost);
            float err = max_abs_error(hC_cpu, hC_kernel);
...

    {
        ...
        dim3 blocks((N + Bn - 1) / Bn, (M + Bm - 1) / Bm);
        cudaMemcpy(dC, hC.data(), sizeof(float) * sizeC, cudaMemcpyHostToDevice);
        v6_gemm_double_buffer<Bm, Bn, Bk, Tm, Tn, THREADS, 4><<<blocks, threads>>>(dA, dB, dC, M, K, N, alpha, beta);
        cudaDeviceSynchronize();
        if (check_correctness) {
            cudaMemcpy(hC_kernel.data(), dC, sizeof(float) * sizeC, cudaMemcpyDeviceToHost);
            float err = max_abs_error(hC_cpu, hC_kernel);
